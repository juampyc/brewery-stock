/* Frontend logic */
const API_BASE = "https://script.google.com/macros/s/AKfycbyWmAeC5QYATjPOVXfhdv74tHGcjjC4UEZbINgBz0egp3WM27jJiCL5aDdo2pT3ASIS/exec";
...
function initTheme(){
  ...
  if (sw){ sw.checked = saved === "dark"; sw.addEventListener("change", ()=>{
    const theme = sw.checked ? "dark":"light"; document.documentElement.setAttribute("data-theme", theme); localStorage.setItem("theme", theme);
  });}
}
...
function shortenUsedRefs(text){ if(!text) return ""; return String(text).replace(/usados:([0-9a-f\-:_,]+)/gi,(m,g)=>g.split(",").map(p=>{const [id,q]=p.split(":");return `${(id||"").slice(-6)}:${q}`}).join(",")); }

async function renderIndex(){
  try{
    const [styles, cans, emptycans, labels] = await Promise.all([apiGet("styles"), apiGet("cans"), apiGet("emptycans"), apiGet("labels")]);
    ...
    const cardC=document.getElementById("idx_total_cans"), cardE=document.getElementById("idx_total_emptycans"), cardL=document.getElementById("idx_total_labels");
...
function renderMovementsTable(list){
  ...
  for(const row of list){
    ...
    tr.innerHTML = `<td>${renderIdShort(row.id)}</td><td>${renderDateLocal(row.dateTime)}</td><td>${row.entity||""}</td><td>${row.type||""}</td><td>${row.qty??0}</td><td>${desc}</td><td>${renderDateLocal(row.lastModified)}</td>`;
...
async function loadProductionData(){
  ...
  const byStyle=new Map(); for(const s of styles) byStyle.set(String(s.id),{style:s, totals:{final:0,pasteurizada_sin_etiquetar:0,sin_pasteurizar_etiquetada:0,sin_pasteurizar_sin_etiquetar:0}});
...
function renderProductionTable(rows){
  ...
  for(const r of rows){
    const {style, totals}=r; const tr=document.createElement("tr");
    tr.innerHTML = `<td>${style.brandName||""}</td><td>${style.name||""}</td>
...
async function openRegisterProduction(styleId){
  try{
    ...
    const labelOptsForStyle = labels.filter(l=>!l.isCustom && String(l.styleId)===String(style.id)).map(l=>`<option value="${l.id}">${l.styleName} (estilo)</option>`).join("");
    ...
    const result=await Swal.fire({ title:"Registrar producción", html, focusConfirm:false, showCancelButton:true,
    ...
    Toast.fire({icon:"success", title:"Producción registrada"}); await loadProductionData();
...
async function openTransition(styleId){
  try{
    ...
    const result=await Swal.fire({ title:"Cambiar estado", html, focusConfirm:false, showCancelButton:true,
    ...
    Toast.fire({icon:"success", title:"Estado actualizado"}); await loadProductionData();
...
function labelModalBody(brands, styles, d={}){
  ...
  <div class="row g-2 mt-1">
    <div class="col-sm-6"><label class="form-label fw-semibold">Nombre (custom opcional)</label><input id="lblName" class="form-control" value="${d.name||""}" placeholder="(si es personalizada)"></div>
    <div class="col-sm-3"><label class="form-label fw-semibold">Cantidad</label><input id="lblQty" type="number" class="form-control" value="${d.qty||0}"></div>
  ...
  <div class="row g-2 mt-1">
    <div class="col-sm-6"><label class="form-label fw-semibold">Proveedor</label><input id="lblProvider" class="form-control" value="${d.provider||""}"></div>
    <div class="col-sm-6"><label class="form-label fw-semibold">Lote</label><input id="lblLot" class="form-control" value="${d.lot||""}"></div>
  </div>
  <div class="form-check mt-2"><input class="form-check-input" type="checkbox" id="lblIsCustom" ${d.isCustom?"checked":""}><label class="form-check-label" for="lblIsCustom">Es personalizada (marca/estilo pueden no coincidir)</label></div>`;
}
async function loadLabelsPage(){
  ...
  for(const l of labels){
    const tr=document.createElement("tr");
    tr.innerHTML = `<td>${renderIdShort(l.id)}</td><td>${brandMap.get(String(l.brandId))?.name||""}</td><td>${l.isCustom?(l.name||"custom"):(styleMap.get(String(l.styleId))?.name||"")}</td><td>${l.qty||0}</td><td>${l.lot||""}</td><td>${l.provider||""}</td><td>${renderDateLocal(l.dateTime)}</td><td>${renderDateLocal(l.lastModified)}</td><td class="text-nowrap"><button class="btn btn-sm btn-outline-secondary me-1" data-id="${l.id}">Editar</button><button class="btn btn-sm btn-danger" data-id="${l.id}">Eliminar</button></td>`;
    tbody.appendChild(tr);
  }
  document.getElementById("btnAddLabel")?.addEventListener("click", async ()=>{
    ...
    if(!isConfirmed) return; await apiPost("labels", value, "create"); Toast.fire({icon:"success", title:"Agregado"}); loadLabelsPage();
  });
  tbody.addEventListener("click", async e=>{
    const btn=e.target.closest("button"); if(!btn) return; const id=btn.dataset.id;
    if(btn.classList.contains("btn-danger")){ await apiDelete("labels", id); Toast.fire({icon:"success", title:"Eliminado"}); return loadLabelsPage(); }
    else { const l=labels.find(x=>String(x.id)===String(id)); const {value,isConfirmed}=await Swal.fire({ title:"Editar etiqueta", html:labelModalBody(brands, styles, l), showCancelButton:true, focusConfirm:false,
      preConfirm:()=>({ brandId:document.getElementById("lblBrandId").value, styleId:document.getElementById("lblStyleId").value, name:document.getElementById("lblName").value, qty:Number(document.getElementById("lblQty").value||0), dateTime:fromDatetimeLocalValue(document.getElementById("lblDt").value), provider:document.getElementById("lblProvider").value, lot:document.getElementById("lblLot").value, isCustom:document.getElementById("lblIsCustom").checked }) });
      if(!isConfirmed) return; await apiPost("labels", {id, ...value}, "update"); Toast.fire({icon:"success", title:"Guardado"}); loadLabelsPage();
    }
  });
}
...
async function boot(){
  ...
  if(document.getElementById("prod_table")) await (loadProductionData(), document.getElementById("btnNewProduction")?.addEventListener("click", ()=> openRegisterProduction("")));
